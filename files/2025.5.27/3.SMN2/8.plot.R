...
internal_ctrl <- read.csv("./3.analysis/7.parse/WV2JYX_13_13_Ctrl_DMSO_80C_5min_Mn_cut_trimmed.mpileup.csv", 
                          header = TRUE)
...
for (SMN2_80C in SMN2_80Cs) {
  ...
  filename <- gsub("./3.analysis/7.parse/WV2JYX_","",SMN2_80C)
  filename <- gsub("_", "-", filename)
  filename <- gsub(".mpileup.csv","",filename)
  filename <- paste0(filename, "_", region)
  filename <- gsub("-cut-trimmed", "", filename)
...
summary_long$Group <- factor(summary_long$Group, levels = c("Base",
                                                            "13_13_Ctrl_DMSO_80C_5min_Mn_cut_trimmed",
                                                            "14_14_Ctrl_C12_0.1mM_80C_5min_Mn_cut_trimmed",
                                                            "15_15_Ctrl_C15_0.1mM_80C_5min_Mn_cut_trimmed"
))
...
colors <- c("avg_A" = "#1F77B4", 
            ...
            "avg_G" = "#2CA02C",
            "avg_U" = "#D62728")
p <- ggplot(sum2, aes(x = Group, y = Value, fill = Base)) +
...
ggsave(paste0("./3.analysis/8.plot/relative/base_summary.jpg"), plot = p, width = 5, height = 5, units = "in", dpi = 1200)
