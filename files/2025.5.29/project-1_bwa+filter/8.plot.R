...
internal_ctrl <- read.csv("./3.analysis/8.plot/4.relative_mut/summary/02_cell_DMSO_37C-15min_Mn.mean.csv", 
                          header = TRUE)
...
for (SL5_37C in SL5_37Cs) {
  ...
  filename <- gsub("./3.analysis/8.plot/4.relative_mut/summary/","",SL5_37C)
  filename <- gsub("_", "-", filename)
...
internal_ctrl <- read.csv("./3.analysis/8.plot/4.relative_mut/summary/04_cell_DMSO_80C-15min_Mn.mean.csv", 
                          header = TRUE)
internal_ctrl <- filter(internal_ctrl, Region == "SARS-CoV2-5UTR-Amplicon-short-trimmed+3")
internal_ctrl$Mean_mut_percentage[internal_ctrl$Mean_mut_percentage < 0] <- 0
...
for (SL5_80C in SL5_80Cs) {
  ...
  filename <- gsub("./3.analysis/8.plot/4.relative_mut/summary/","",SL5_80C)
  filename <- gsub("_", "-", filename)
  filename <- gsub(".mean.csv","",filename)
  filename <- paste0(filename, "_", region)
...
# ggsave(paste0("./3.analysis/8.plot/5.base_summary/base_summary.pdf"), plot = p, width = 12, height = 5, units = "in", dpi = 1200)
ggsave(paste0("./3.analysis/8.plot/5.base_summary/base_summary.jpg"), plot = p, width = 12, height = 5, units = "in", dpi = 1200)
