...
SMN2_80Cs <- csvs[grepl("80C-5min", csvs)]
internal_ctrl <- read.csv("./3.analysis/7.parse/13-Ctrl-DMSO-80C-5min-Mn_001.mpileup.csv", 
                          header = TRUE)
...
for (SMN2_80C in SMN2_80Cs) {
  ...
  filename <- gsub("./3.analysis/7.parse/","",SMN2_80C)
  filename <- gsub("_", "-", filename)
  filename <- gsub(".mpileup.csv","",filename)
  filename <- paste0(filename, "_", region)
...
names <- gsub("_001.mpileup.csv", "", basename(SMN2_80Cs))
...
summary_long$Group <- factor(summary_long$Group, levels = c("Base",
                                                            "13-Ctrl-DMSO-80C-5min-Mn",
                                                            "14-Ctrl-C12-100uM-80C-5min-Mn",
                                                            "15-Ctrl-C15-100uM-80C-5min-Mn"
...
#ggsave(paste0("./3.analysis/8.plot/relative/base_summary_ctrl.pdf"), plot = p, width = 6, height = 6, units = "in", dpi = 1200)
ggsave(paste0("./3.analysis/8.plot/relative/base_summary_ctrl.jpg"), plot = p, width = 6, height = 6, units = "in", dpi = 1200)
